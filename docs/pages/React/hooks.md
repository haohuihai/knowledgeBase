...
对比之后，就会产生patch；React 仅在渲染之间存在差异时才会更改 DOM 节点

在react 渲染完成并且更新DOM后，浏览器就会重新绘制屏幕
...
只要state发生变化，不管界面上是否存在state， 都会重新渲染；


当 React 重新渲染一个组件时：

React 会再次调用你的函数（组件）
函数会返回新的 JSX 快照 （修改后的一个对象）
React 会更新界面以匹配返回的快照 （这个对象和之前的对象进行比较，更新页面）

调用组件时：返回 state 快照 和UI快照，快照中的变量，都是那一次渲染中state中的值计算出来的；即返回后的快照，已经是更新后的对象；



memo()

子组件没有需要渲染的变量，使用memo()包裹的组件不会重新渲染；不包裹，父组件有state变量更改，子组件也会渲染