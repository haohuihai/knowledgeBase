...
- **稳定性**：单进程应用崩溃可能导致整个应用失效，而多进程应用中，一个进程崩溃不会影响其他进程。
- **并发性**：单进程应用只能顺序执行，而多进程应用可以同时执行多个任务。
- **调试**：单进程中的多线程调试困难，而多进程中的多线程调试相对容易。
- **性能**：单进程的容量受限，多进程的容量不受限。
- **开发**：单进程内模块间是强依赖关系，需要在一起编译，相互影响较大，不利于多团队并行开发。多进程更便于多语言的协作开发。

Chrome浏览器, 是多进程架构, 先来看一下都包含哪些进程。
...
https://cloud.tencent.com/developer/article/2079056
